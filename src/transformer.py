...
class Transformer(nn.Module):
    ...
    def __greedy_decode(self, src_input: torch.Tensor, max_length: int, **kwargs) -> torch.Tensor:
        ...
        enc_output = self.encoder(src_input, attn_mask)  # (batch_size, src_seq_len, d_model)
        ...
        for _ in range(max_length):
            ...
            dec_output = self.decoder(tgt_input, enc_output)  # (batch_size, seq_len, d_model)
            
            # Project the decoder output to vocabulary size
            dec_output = self.output_linear(dec_output)  # (batch_size, seq_len, vocab_size)
    ...
    def __beam_search_decode(self, src_input: torch.Tensor, max_length: int, beam_size: int = 3, **kwargs) -> torch.Tensor:
        ...
        # Pass the source input through the encoder
        attn_mask = kwargs.get('attn_mask', None)
        enc_output = self.encoder(src_input, attn_mask)  # (1, src_seq_len, d_model)
        ...
        for _ in range(max_length):
            ...
            for seq, score in beam:
                ...
                # Consider top-k tokens for beam expansion
                topk_log_probs, topk_indices = log_probs.topk(beam_size, dim=-1)  # (1, k)
                for i in range(beam_size):
                    next_token = topk_indices[0, i].unsqueeze(0).unsqueeze(0)  # (1, 1)
                    new_seq = torch.cat([seq, next_token], dim=1)  # (1, seq_len+1)
    ...
    def __sampling_decode(self, src_input: torch.Tensor, max_length: int, temperature: float = 1.0, **kwargs) -> torch.Tensor:
        ...
        for _ in range(max_length):
            ...
            tgt_input = torch.cat([tgt_input, next_token], dim=1)  # (batch_size, seq_len+1)
        ...
        # Return the generated sequences (excluding the first SOS token)
        generated_sequence = tgt_input[:, 1:]  # (batch_size, generated_seq_len)
        return generated_sequence
    ...
    def __top_k_sampling_decode(self, src_input: torch.Tensor, max_length: int, k: int = 10, **kwargs) -> torch.Tensor:
        ...
        # Initialize the target sequence with SOS_token
        tgt_input = torch.full((batch_size, 1), SOS_token, dtype=torch.long, device=device)  # (batch_size, src_seq_len, d_model)
        ...
        for _ in range(max_length):
            ...
            # Apply log softmax to get log probabilities
            log_probs = F.softmax(logits, dim=-1)  # (batch_size, vocab_size)
            log_probs = torch.clamp(log_probs, min=1e-9)
            
            # Get the top k tokens
            topk_log_probs, topk_indices = torch.topk(log_probs, k, dim=-1)  # (batch_size, k)
            
            # Sample from the top k tokens
            probs = topk_log_probs / topk_log_probs.sum(dim=-1, keepdim=True)  # (batch_size, k)
            probs = torch.clamp(probs, min=1e-9)
            
            next_token_idx = torch.multinomial(probs, num_samples=1)  # Shape: (batch_size, 1)
    ...
    def __top_p_sampling_decode(self, src_input: torch.Tensor, max_length: int, p: float = 0.9, **kwargs) -> torch.Tensor:
        ...
        for _ in range(max_length):
            ...
            next_token_idx = torch.multinomial(sorted_probs, num_samples=1)  # (batch_size, 1)
            
            # Map sampled indices to original token indices
            next_token = sorted_indices.gather(-1, next_token_idx)  # (batch_size, 1)
    ...
    def __contrastive_decode(self, src_input: torch.Tensor, max_length: int, k: int = 5, alpha: float = 0.6, **kwargs) -> torch.Tensor:
        ...
        for _ in range(max_length):
            ...
            expanded_tgt_input = tgt_input.repeat(1, k).view(batch_size * k, -1)  # Shape: (k, seq_len)
            ...
            dec_outputs_candidate = self.decoder(y_candidates, enc_output.repeat(k, 1, 1))  # (batch_size*k, seq_len+1, d_model)
        ...
        generated_sequence = tgt_input[:, 1:]  # (batch_size, seq_len)
        return generated_sequence
