...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self, d_model: int, num_attention_heads: int):
        ...
        self.heads = nn.ModuleList([
            AttentionHead(d_model, d_k, d_k, d_v)
    ...
    def forward(self, x_q, x_k, x_v, mask=None):
        ...
        for head in self.heads:
            out = head(x_q, x_k, x_v, mask)  # each head returns (output, weights)
            head_outputs.append(out)
            
        # Concatenate the outputs from all attention heads.
        x = torch.cat(head_outputs, dim=-1)  # shape: (batch_size, seq_len, num_heads * d_v)
